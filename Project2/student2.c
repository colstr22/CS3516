...
void B_input(struct pkt packet) {
  // Check for out-of-order packets, can also drop a corrupted header
  ...
  if (packet.checksum != get_checksum(&packet)) {
    ...
    // Drop it and wait for timeout, when packets will be resent
    return;
  }
